  ...
  TOUCH_CONTROLLER: 'controller'
    ...
    Runner.prototype = {
        ...
        doAction: function(action){
            if (action == 'UP') {
                this.onKeyDownAuto('38');
                this.onKeyUpAuto('38');
            } else if (action == 'DOWN') {
                this.onKeyDownAuto('40');
                this.onKeyUpAuto('40');
            }
        ...
        getNextMove: function() {
            ...
            var canvas = $(".runner-canvas").get(0);
            var img = canvas.toDataURL("image/png");
            // send data to server
            $.post("receiver", {'data':img, 'reward': Math.round(this.distanceRan * 0.025) , 'state': this.crashed}, this.doAction.bind(this));
        ...
        onKeyDownAuto: function(keyCode) {
            if (IS_MOBILE) {
                e.preventDefault()
            }

            if (!this.crashed && (Runner.keycodes.JUMP[keyCode])) {
                if (!this.activated) {
                    this.loadSounds();
                    this.activated = true;
                }
            ...
            if (this.crashed && keyCode == "32"){ //&& e.currentTarget == this.containerEl) {
                this.restart();
            }
            if (this.activated && !this.crashed && Runner.keycodes.DUCK[keyCode]) {
                e.preventDefault();
                if (this.tRex.jumping) {
                    this.tRex.setSpeedDrop();
                } else if (!this.tRex.jumping && !this.tRex.ducking) {
        ...
        onKeyUpAuto: function(keyCode) {
            ...
            if (isjumpKey) {
                ...
                var deltaTime = getTimeStamp() - this.time;
                if (Runner.keycodes.RESTART[keyCode] || (deltaTime >= this.config.GAMEOVER_CLEAR_TIME && Runner.keycodes.JUMP[keyCode])) {
                    this.restart()
                }
            } else if (this.paused && isjumpKey) {
                this.tRex.reset();
...
Runner.prototype = {
  ...
  /**
   * updateAuto to enable control from the server. Update game frame and schedules the next one.
   */
  updateAuto: function() {
    this.updatePending = false;

    var now = getTimeStamp();
    var deltaTime = this.msPerFrame;
    this.time = now;
  ...
  getNextMove: function() {
      ...
      var img = canvas.toDataURL("image/png");
      // send data to server
      $.post("receiver", {'data':img, 'reward': Math.ceil(this.distanceRan) , 'state': this.crashed}, this.doAction.bind(this));
  ...
  onKeyDownAuto: function(keyCode) {
    ...
    } else if (this.crashed && keyCode == "32") {
      this.restart();
    }
  },
  ...
  onKeyUpAuto: function(keyCode) {
    ...
    if (this.isRunning() && isjumpKey) {
      ...
      if (Runner.keycodes.RESTART[keyCode] || (deltaTime >= this.config.GAMEOVER_CLEAR_TIME &&
          Runner.keycodes.JUMP[keyCode])) {
        this.restart();
      }
    } else if (this.paused && isjumpKey) {
